...
```bash
npx -y @memi-design/cli@2.7.2 agent brief . --agent hermes --intent "<interface task>" --detail compact --json
```

Run only the evidence command needed next:

```bash
npx -y @memi-design/cli@2.7.2 diagnose . --json --no-write --fail-on none
npx -y @memi-design/cli@2.7.2 ux audit . --json --no-write
npx -y @memi-design/cli@2.7.2 tokens --from ./src --report --json
```
