...
`memi design CI` is the supported GitHub distribution surface for the deterministic `memi ci` gate. It defaults to the reviewed `@memi-design/cli@2.7.3` pin, scans the checked-out repository, writes a GitHub step summary, uploads design-health evidence, and can publish SARIF annotations to code scanning.
...
| --- | --- | --- |
| `version` | `2.7.3` | Exact CLI version. The reviewed default is pinned; exact prior versions remain accepted for existing workflows. Ranges and dist-tags are rejected. |
...
4. Create the immutable `v2.7.3` release tag on that commit. Never retag an existing release.
