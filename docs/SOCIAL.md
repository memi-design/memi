...
Current release: `@memi-design/cli@2.7.4`
...
I built Memi to give the agent a deterministic preflight:
  ...
  npx -y @memi-design/cli@2.7.4 diagnose . --json --no-write --fail-on none
...
npx -y @memi-design/cli@2.7.4 diagnose . --no-write
...
```text
Memi 2.7.4: one read-only command before your agent edits UI
```

Body:

```text
Memi 2.7.4 is published on npm, GitHub Releases, the v2 Action channel, and
GHCR. Pinned Studio assets are verified; MCP Registry and website parity remain
explicitly pending.
