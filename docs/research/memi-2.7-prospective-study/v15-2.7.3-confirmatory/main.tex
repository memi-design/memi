...
\begin{abstract}
This study does not establish that Memi is better. Coding agents can receive repository-specific design skills, but successful
...
\subsection{What V15 establishes---and what it does not}

The short answer is deliberately narrow. V15 supports one bounded statement:
on the two tasks with complete rendered evidence, the data rule out a design-
quality loss of five or more rubric points under the preregistered model-panel
procedure. It does not establish that Memi is better, generally faster, or
cheaper. The third task has no admissible quality estimate, the quality panel
contains only 10 complete pairs, none of the resource tests survived
multiplicity correction, and billing observations were not collected.

\begin{figure*}[t]
\centering
\ClaimDecisionFigure
\caption{Plain-language claim boundary for V15. Clearing the orange $-5$ point
margin means that a large decline was not observed on the two gradable tasks;
it is not evidence that Memi outperforms the baseline.}\label{fig:claim-decision}
\end{figure*}

\subsection{All cells were admitted, but quality evidence covered only two tasks}
...
\subsection{\RQ{1}: The study rules out a large decline, not a performance win}

Figure~\ref{fig:quality} separates observed pair differences from the
task-level decision statistic. The orange $-5$ line is an acceptable-loss
boundary, not a threshold for winning: a decision bound to its right passes
non-inferiority, while a mean to the right of zero still does not prove better
performance. Buzzr's mean difference was $+1.4$ points
...
\centering
\QualityFigure
\caption{Blinded design-quality evidence. Dots are complete pairs, pale spans
are two-sided 95\% paired-bootstrap intervals, solid dots are task means, and
vertical ticks are the one-sided decision bounds. The orange reference is the
$-5$ point acceptable-loss margin; the zero line denotes equality. Passing the
orange line means ``no large decline observed,'' not ``Memi is better.''}
\label{fig:quality}
\end{figure*}
...
This policy is a safety mechanism, not evidence that a skill improves a future
frontend. Its purpose is to prevent weak, stale, or mismatched evidence from
turning a skill into a default route.
...
\subsection{\RQ{3}: Replay confirmed policy mechanics, not product efficacy}
...
Paraform/\texttt{design-extract} was suppressed by its first exact event. Buzzr/
...
providers, or skill revisions. The detailed chronological replay remains in
the reproducibility package because it validates timing and isolation, rather
than a user-facing quality or efficiency outcome.
...
\section*{What must change before a better, faster, or cheaper claim}

The next study must use native rendered evidence on every platform, a
preregistered independent human practitioner panel, measured billing records,
and a larger confirmatory sample sized before execution. It must separately
test quality superiority and efficiency, rather than treating a non-inferiority
pass or provider token count as either result. The accompanying 2.7.5 plan
specifies the harness, skill, router, and release gates required before those
claims may be made; new evidence must remain isolated by exact route identity
and skill-content hash.
